...
| E7 observed regression | Restock evaluation has no defensible concern, passes, but repository refuses merge | Reports `MERGE_REFUSED`, preserving `PASS` and mapping to `approve`; a PASS with a canonical finding maps to `needs-attention`, while WARN/FAIL or PASS without a canonical finding stays in domain routing with no canonical verdict | Bare `BLOCKED`, false `approve`, or unconditional `needs-attention` | block |
