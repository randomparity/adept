...
| SRL-06b | `--reviewer detect-evil` receives an unresolvable target plus a valid trailing charter, while an older artifact exists at the output path | delegated target-resolution error surfaced once; zero malformed-return retry; no compact object or new artifact | stale artifact consumed, invented verdict/disposition, or reviewer re-dispatch | block |
