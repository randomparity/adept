...
| E11 closure failure | Open-sweep occurrence creation succeeds; `gh issue close` fails; readback is `OPEN` with null `stateReason` | Report the occurrence as `OPEN`/`NONE`, persist one valid pending row, and stop before campaign completion | Blank reason, `closed-not-planned` outcome, or continued completion | block |
...
| E11 | bounty/campaign open sweep | E10 packet, but close #121 fails with `permission denied`; readback returns `state: OPEN`, `stateReason: null` |
