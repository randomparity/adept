...
- ~~**adept's `CLAUDE.md`** carrying the four anatomy rules. That belongs with the rewrite, whose rules it states; adding it now would describe a repo that does not yet exist.~~
  **Corrected — this was wrong, and `CLAUDE.md` was written immediately after this plan ran.** All four rules were already true of the repo at the time this deferral was written: the visual companion had been deleted by Task 1 (rule 3) and the prose assertions by Task 2 (rule 4). The document describes what exists, not what is planned. The deferral also had the harm backwards: rules that live only in a spec are rules nothing consults, so the *absence* was the state in which a later session could reintroduce exactly what these tasks removed.
