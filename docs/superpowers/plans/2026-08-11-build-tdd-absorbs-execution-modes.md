...
**Amended during execution.** Task 1 Step 4's completeness pass found four behaviours in the 445-line `subagent-driven-development` source covered by no row: the prompt-template usage (`## Prompt Templates` mapped nowhere), and three from `## Red Flags` — scene-setting in every dispatch, answering a subagent's questions before it proceeds, and never starting on `main`/`master` without consent. Added as rows 41–44, all KEEP. `executing-plans` and `using-git-worktrees` mapped cleanly with no gap. Final totals: `executing-plans` 15 rows (10 KEEP), `using-git-worktrees` 21 rows (16 KEEP), `subagent-driven-development` 44 rows (37 KEEP).
