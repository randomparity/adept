...
the numbers are deliberately left unallocated rather than reused, so
nothing silently re-points at a record that does not exist. Any other
absent number is a gap, not a loss -- records are append-only, and the
records gate fails any disappearance.
