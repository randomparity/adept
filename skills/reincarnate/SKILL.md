...
Prefer executable configuration and current source over descriptive prose when they
conflict. Do not silently turn an observed convention into policy. Mark a claim
`unverifiable` when local evidence cannot settle it, and ask the operator rather than
inventing an answer or searching externally without permission.
...
- **stale** — contradicted by present evidence;
- **unverifiable** — neither confirmed nor contradicted;
- **policy** — an operator-owned rule that evidence cannot repeal merely because the
  repository currently behaves differently.
...
Draft a complete replacement for each selected file. Keep only guidance that helps an
agent act correctly in that scope. Remove duplication, historical explanation, and
generic advice already supplied by a higher-precedence file or the agent harness.
