...
`$forge`'s two reviewer templates — the per-task reviewer and the whole-branch `code-reviewer.md`, both derived from vendored superpowers prompts — grade `Critical / Important / Minor` instead. When one of their findings crosses into a command surface, map it:
