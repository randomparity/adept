...
7. **Decompose mode** (arguments name a parent issue, e.g. "decompose #N"): read the parent and
   ...
   If any required persisted-evidence read or validation fails and that independent derivation
   read also fails, stop before drafting or filing. Report non-adoption, name only the failed
   operation, and give a safe retry action; never include the external payload, authentication
