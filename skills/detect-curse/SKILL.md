---
name: detect-curse
description: "Find the cause of a failure before changing anything. Use on a failing test, a reported bug, a broken build, or behaviour you cannot account for — and before proposing a fix for any of them."
---
...
and `$grimoire` is what fills that directory — skip it and the compounding loop
only ever writes.
...
`$grimoire` to write it to `docs/solutions/` (plus a vestige recall pointer).
...
from scratch. `$grimoire` has its own bar (non-obvious, likely to recur, saves a
